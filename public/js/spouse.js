...
function handleSpouseSearch(event) {
  ...
  if (state) clearTimeout(state.spouseSearchTimeout);
  ...
  if (state) state.spouseSearchTimeout = timeout;
}
...
function forceReload(id) {
  const state = window.IntegrityState;
  if (state) clearInterval(state.pollInterval);
  google.script.run.withSuccessHandler(function(r) { 
...
console.log('SUCCESS: handleSpouseSearch is now global');
console.log('[Spouse Module] All functions exposed to window synchronously');
