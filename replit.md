...
- **Campaign Manager Dashboard (Sprint 4)**: Mega Modal (90vw x 90vh) with 2 tabs: Campaign Dashboard (table with Delivery/Open/Engagement/Unsub rates, clickable rows for detail view, "New Campaign" button creates record in Sidecar Base), Data Tools (2-column grid: Export left, Import right). Detail view shows recipient list with filter toggles (All/Opened/Clicked/Bounced/Unsubscribed), clickable contact names, status badges, and mailto follow-up button for unsubscribed recipients. Backend: `getCampaignStats()` queries Campaigns table then joins with Logs, `getCampaignLogs(campaignId)` returns individual recipient logs with Contact Name lookup, `createCampaign(name)` creates new campaign in Sidecar Base.
- **Export Data Quality**: Excludes Inactive and Deceased contacts from export. Uses Airtable's `Calculated Name` field instead of manual FirstName+LastName concatenation. CSV output includes BOM (`\uFEFF`) for Excel compatibility with special characters (Zoë, O'Connor). Export stats show full breakdown: Total → Less Inactive → Less Deceased → Less Unsubscribed = Marketable = Ready to Send (Unique Emails).